...
function boilerplate (done) {
  ...
  const args = yargs(hideBin(process.argv)).command('curl <url>', 'fetch the contents of the URL', () => {}, (argv) => {
    console.info(argv);
  })
    .demandCommand(1)
    .parse();
  gulp.src(`${PATHS.src}/settings/boilerplate/**/*`)
...
function images (done) {
  ...
  FOLDERS.map(folder => {
    return gulp.src(path.join(PATHS.email_src, folder, '/img/*'))
      .pipe(imagemin([
...
function zipFn (done) {
  ...
  FOLDERS.map(folder => {
    return gulp.src(path.join(PATHS.zipPath, folder, '**/*'))
      .pipe(zip(`${folder}.zip`))
...
const initialDev = gulp.series(images, pugFn);
const initialBuild = gulp.series(images, pugLinter, pugFn, stylesLint);

const dev = gulp.series(index, initialDev, serve, watch);
const build = gulp.series(clean, index, initialBuild);

export { boilerplate, zipFn, build };
export default dev;
